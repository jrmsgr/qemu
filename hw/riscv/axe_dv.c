...
static void create_fdt(AxeDvState *s, const MemMapEntry *memmap,
                       bool is_32_bit, bool htif_custom_base)
{
    ...
    qemu_fdt_setprop_string(fdt, "/", "model", "ucbbar,axe_dv-bare,qemu");
    qemu_fdt_setprop_string(fdt, "/", "compatible", "ucbbar,axe_dv-bare-dev");
    ...
    if (htif_custom_base) {
        qemu_fdt_setprop_cells(fdt, "/htif", "reg",
            0x0, memmap[AXE_DV_HTIF].base, 0x0, memmap[AXE_DV_HTIF].size);
    }
    ...
    for (socket = (riscv_socket_count(ms) - 1); socket >= 0; socket--) {
        ...
        qemu_fdt_setprop_cells(fdt, clint_name, "reg",
            0x0, clint_addr, 0x0, memmap[AXE_DV_CLINT].size);
    ...
    axe_dv_rtl_sim_name = g_strdup_printf("/soc/axe-dv-rtl-sim@%"HWADDR_PRIx,
                           memmap[AXE_DV_AXE_DV_RTL_SIM].base);
    qemu_fdt_add_subnode(ms->fdt, axe_dv_rtl_sim_name);
    qemu_fdt_setprop_sized_cells(ms->fdt, axe_dv_rtl_sim_name, "reg",
                                 2, memmap[AXE_DV_AXE_DV_RTL_SIM].base,
                                 2, memmap[AXE_DV_AXE_DV_RTL_SIM].size);
}

static bool axe_dv_test_elf_image(char *filename)
{
    Error *err = NULL;
...
static void axe_dv_board_init(MachineState *machine)
{
    ...
    for (i = 0; i < riscv_socket_count(machine); i++) {
        ...
        riscv_aclint_swi_create(
            memmap[AXE_DV_CLINT].base + i * memmap[AXE_DV_CLINT].size,
    ...
    memory_region_init_rom(mask_rom, NULL, "riscv.axe_dv.mrom",
                           memmap[AXE_DV_MROM].size, &error_fatal);
    memory_region_add_subregion(system_memory, memmap[AXE_DV_MROM].base,
                                mask_rom);
    ...
    if (!firmware_name && !machine->kernel_filename) {
        ...
        if (firmware_name) {
            htif_custom_base = !axe_dv_test_elf_image(firmware_name);
        }
        if (!htif_custom_base && machine->kernel_filename) {
            htif_custom_base = !axe_dv_test_elf_image(machine->kernel_filename);
        }
    }
    ...
    fdt_load_addr = riscv_compute_fdt_addr(memmap[AXE_DV_DRAM].base,
                                           memmap[AXE_DV_DRAM].size,
                                           machine, &boot_info);
    riscv_load_fdt(fdt_load_addr, machine->fdt);

    /* load the reset vector */
    riscv_setup_rom_reset_vec(machine, &s->soc[0], firmware_load_addr,
                              memmap[AXE_DV_MROM].base,
                              memmap[AXE_DV_MROM].size, kernel_entry,
                              fdt_load_addr);

    /* initialize HTIF using symbols found in load_kernel */
    htif_mm_init(system_memory, serial_hd(0), memmap[AXE_DV_HTIF].base,
                 htif_custom_base);
}

static void axe_dv_set_signature(Object *obj, const char *val, Error **errp)
{
...
static void axe_dv_machine_class_init(ObjectClass *oc, const void *data)
{
    ...
    mc->default_ram_id = "riscv.axe_dv.ram";
    object_class_property_add_str(oc, "signature", NULL, axe_dv_set_signature);
...
{
    type_register_static(&axe_dv_machine_typeinfo);
}

type_init(axe_dv_machine_init_register_types)
